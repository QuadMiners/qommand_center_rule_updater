...
class MegaRaidManager(object):
    ...
    def __init__(self, extract_data=False, conn=None):
        ...
        try:
            ...
            logger.error("Mega Raid Check Exception  | {0}".format(er))
    ...
    def __get_disk_name(self, vid):
        ...
        try:
            ...
            if error != b'':
                logger.error("[RAID Physical Drive] Get Device Name Excute Error msg | {}".format(error))

            ret = output.decode('utf-8').strip().split(' ')[-1].replace('../../', '/dev/')

        except OSError as e:
            logger.error("[RAID Physical Drive] Get Device Name Export OSError | {}".format(e))

        except subprocess.SubprocessError as e:
            logger.error("[RAID Physical Drive] Get Device Name Subprocess Error | {}".format(e))
    ...
    def __get_pd(self):
        ...
        for pd in pd_data:
            ...
            if state != 'ok':
                logger.error(f'[RAID Physical Drive] status is not ok: {state}')
                state = 'failed'
    ...
    def __get_ld(self):
        ...
        for ld in ld_data:
            ...
            if state != 'ok':
                logger.error(f'[RAID Logical Drive] status is not ok: {state}')
                state = 'failed'
    ...
    def __upsert_raid_info_pd(self):
        for pd in self.pd_data:
            ...
            if self.conn:
                ...
                if pd['disk_state'] != 'ok' or pd['bbu_status'] != 'optimal':
                    self.history_error(92000, **dict(server_id=gconfig.server_id, disk=f'physical drive {pd["device_id"]}', status=f'{pd["disk_state"]}(battery: {pd["bbu_status"]})'))
...
if __name__ == '__main__':
    ...
    a = MegaRaidManager(extract_data=True)
