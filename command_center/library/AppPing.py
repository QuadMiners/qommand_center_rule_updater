...
def do_one(myStats, destIP, hostname, timeout, mySeqNumber, packet_size, quiet=False):
    ...
    try:  # One could use UDP here, but it's obscure
        ...
        logger.error("failed. (socket error: '%s')" % e.args[1])
    ...
    if recvTime:
        ...
        if not quiet:
            ...
            logger.info("%d bytes from %s: icmp_seq=%d ttl=%d time=%d ms" % ( dataSize, socket.inet_ntoa(struct.pack("!I", iphSrcIP)), icmpSeqNumber, iphTTL, delay) )
        myStats.pktsRcvd += 1
        ...
        logger.info("%s : Request timed out." % destIP)
...
def verbose_ping(hostname, timeout=WAIT_TIMEOUT, count=NUM_PACKETS,
                 packet_size=PACKET_SIZE, path_finder=False, server=None):
    """
    ...
    try:
        ...
        print("\nPYTHON PING %s (%s): %d data bytes" % (server and server.host_name or hostname, destIP, packet_size))
    except socket.gaierror as e:
        print("\nPYTHON PING: Unknown host: %s (%s)" % (server and server.host_name or hostname, e.args[1]))
        print()
...
def quiet_ping(hostname, timeout=WAIT_TIMEOUT, count=NUM_PACKETS,
    ...
    if path_finder:
        fakeStats = MyStats()
        do_one(fakeStats, destIP, hostname, timeout, mySeqNumber, packet_size)
        time.sleep(0.5)
    ...
    return myStats.maxTime, myStats.minTime, myStats.avrgTime, myStats.fracLoss, fail_cnt
