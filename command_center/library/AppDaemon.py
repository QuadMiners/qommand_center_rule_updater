...
class Daemon(object):
    ...
    def __init__(self, pidfile, stdin=os.devnull, stdout=os.devnull, stderr=os.devnull,
                             home_dir='.', verbose=1, use_gevent=False):
        self.stdin = stdin
        self.stdout = stdout
        self.stderr = stderr
    ...
    def daemonize(self):
        ...
        def sigtermhandler(signum, tirum):
            logger.info('Signal handler called with signal {0}'.format(signum))
            self.daemon_alive = False
            try:
                # 프로그램 종료시에 redis 부분을 종료하기 위함. 정상적 종료를 위해
                if self.rconn:
                    self.psub.close()
                    self.rconn.close()
                    self.psub = None
                ...
                logger.info("Signal rconn not in   | {0}".format(k))
        ...
        pid = str(os.getpid())
        pidpath = "/".join(self.pidfile.split('/')[:-1])
        if os.access(pidpath, os.R_OK) is False:
            os.makedirs(pidpath,mode=0o777, exist_ok=False)
